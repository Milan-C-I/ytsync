            ...
            <Card className="bg-neutral-900/80 border-gray-700">
              <CardHeader>
                <CardTitle className="flex items-center text-white gap-2">
                  ...
                  Video Player
                  {/* {videoId && (
                    <span className="text-sm text-white-400 ml-auto">
              ...
              <CardContent className="space-y-4">
                ...
                {videoId && (
                  <div className="text-center text-sm text-gray-400">
                    All video controls (play, pause, seek, forward, backward) are synchronized across all users
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          <motion.div className="lg:col-span-1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <Card className="bg-neutral-900/80 border-gray-700 h-full flex flex-col">
              ...
              <CardContent className="flex-1 flex flex-col space-y-4">
                ...
                <div className="flex-1 overflow-y-auto max-h-80 bg-neutral-800/50 p-3 rounded-lg space-y-2 scrollbar-thin scrollbar-thumb-gray-600">
                  {chat.length === 0 ? (
                    <p className="text-gray-400 text-center text-sm">No messages yet...</p>
                  ) : (
                    chat.map((msg, index) => (
                      <div key={index} className="p-2 rounded-md bg-gray-700/50 text-sm break-words">
                        {msg}
                      </div>
                    ))
                  )}
